...
def _business_hours_from_rows(rows: list) -> dict:
    """Monta dict legível para prompts a partir de clinica_business_hours."""
    ...
    with get_connection() as conn:
        ...
        cur.execute(
            ...
            SELECT
                ...
                c.plan,
                p.assistant_name,
                p.phone         AS clinica_phone,
                p.address       AS clinica_address,
                p.features
            FROM saas_companies c
            JOIN clinica_profile p ON p.company_id = c.id
        ...
        return {
            ...
            "clinica_phone": company["clinica_phone"],
            "clinica_address": company["clinica_address"],
