...
def build_escalation_tools(company_id: int, client_id: str) -> list:
    ...
    def escalate_to_human(summary: str, last_message: str) -> dict:
        ...
        with get_connection() as conn:
            ...
            # 2. Busca o telefone do dono da clínica
            cur.execute(
                "SELECT owner_phone FROM clinica_profile WHERE company_id = %s",
                (company_id,),
            )
            clinica_row = cur.fetchone()
            owner_phone = clinica_row["owner_phone"] if clinica_row else None
