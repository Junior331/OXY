...
def _requires_consecutive_slots(service_row: dict, pacienterow: dict) -> bool:
    ...
    if m <= 1:
        return False
    size = (pacienterow.get("size") or "").strip().upper()
    return size in ("G", "GG")
...
def build_booking_tools(company_id: int, client_id) -> list:
    ...
    def get_specialties() -> dict:
        """
        Retorna especialidades ativas do clinica.
    ...
    def get_services() -> dict:
        """
        Retorna lista de serviços ativos do clinica.
        Chamar em silêncio para validar o serviço antes de pedir dados do paciente.
        """
        with get_connection() as conn:
            cur = conn.cursor()
            cur.execute(
    ...
    def _query_available_slots(specialty_id_val, target_date_val):
        ...
        do serviço — fonte: view `vw_slot_availability` (clinica_slots + specialty ativa).
        """
        with get_connection() as conn:
            cur = conn.cursor()
            cur.execute(
    ...
    def get_available_times(
        ...
        Args:
            ...
            pacienteid: UUID do paciente — use junto com service_id para aplicar a regra de dois slots consecutivos
        """
        try:
        ...
        if not _is_uuid(spec_id):
            ...
            if not _is_uuid(spec_id):
                ...
                return {
                    ...
                    "hint": "Chame get_services, use o campo specialty_id do serviço escolhido, ou só service_id + target_date + pacienteid.",
                }
        ...
        if _is_uuid(pacienteid):
            with get_connection() as conn:
                cur = conn.cursor()
                cur.execute(
                    """
                    SELECT size FROM clinica_pacientes
                    WHERE id = %s AND client_id = %s AND company_id = %s AND is_active = TRUE
                """,
                    (pacienteid, client_id, company_id),
                )
                pacienter = cur.fetchone()
            if pacienter and (pacienter.get("size") or "").strip().upper() in ("G", "GG"):
                sid = None
                if service_id is not None:
                    try:
                        sid = int(service_id)
                ...
                if (
                    inferred_svc_row
                    and pacienter
                    and _requires_consecutive_slots(inferred_svc_row, pacienter)
                ):
        ...
        if not available_slots:
            return {
                ...
                "available_times": [],
                "message": (
                    "Para este paciente/serviço é necessário **dois slots seguidos** com vaga; "
    ...
    def create_appointment(
        ...
        logger.info(
            "create_appointment | client_id=%s | pacienteid=%s | service_id=%s | slot_id=%s",
            client_id,
            pacienteid,
            service_id,
            slot_id,
        ...
        try:
            ...
            logger.exception(
                "create_appointment falhou | client_id=%s | pacienteid=%s | service_id=%s | slot_id=%s | erro=%s",
                client_id,
                pacienteid,
                service_id,
                slot_id,
    ...
    def _do_create_appointment(
        company_id, client_id, pacienteid, service_id, slot_id, notes
    ):
        ...
        with get_connection() as conn:
            ...
            # Valida pacienteid — busca também campos obrigatórios para agendamento
            cur.execute(
                "SELECT id, size, species, breed FROM clinica_pacientes WHERE id = %s AND client_id = %s AND company_id = %s AND is_active = TRUE",
                (pacienteid, client_id, company_id),
            )
            pacienterow = cur.fetchone()
            if not pacienterow:
                return {
                    "success": False,
            ...
            need_double = _requires_consecutive_slots(service_row, pacienterow)
            second_row = None
            if need_double:
                cur.execute(
                    """
                ...
                if idx >= len(day_slots) - 1:
                    ...
                    return {
                        ...
                        "message": (
                            "Este serviço exige dois horários seguidos para pacientes G/GG; "
            ...
            if not need_double:
                cur.execute(
                    """
                    INSERT INTO clinica_appointments
                        (company_id, client_id, pacienteid, service_id, slot_id,
            ...
            if need_double and second_row:
                ...
                success_payload["customer_pickup_hint"] = (
                    ...
                    f"(para buscar o paciente, combine com o clinica — em geral após {end_br})."
                )
            else:
        ...
        with get_connection() as conn:
            ...
            if (
                ...
                if partner_id:
                    cur.execute(
                        """
                        SELECT status FROM clinica_appointments
                        WHERE id = %s AND company_id = %s AND client_id = %s
                    """,
