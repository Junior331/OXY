            ...
            out.append(
                {
                    ...
                    "pacientename": first.get("pacientename"),
                    "start_time": t1,
        ...
        out.append(
            {
                ...
                "pacientename": r.get("pacientename"),
                "start_time": st,
...
def build_client_tools(company_id: int, client_id: str) -> list:
    ...
    def create_pet(name: str, species: str, breed: str, size: str) -> dict:
        ...
        o nome/apelido do paciente antes de prosseguir. O nome é o apelido dado pelo dono
        (ex: "Rex", "Bolinha", "Max", "Luna", "Mel", "Toby").

        Args:
            name: Nome/apelido do paciente dado pelo dono (NÃO pode ser uma raça)
        ...
        if name.strip().lower() in _KNOWN_BREEDS:
            return {
                ...
                "name_is_breed": True,
                "message": (
                    f"'{name}' parece ser uma raça, não um nome de paciente. "
                    "Pergunte ao cliente: 'Qual é o nome/apelido do seu paciente?' antes de cadastrar."
                ),
            }
        ...
        if not size_db:
            return {
                ...
                "message": "Porte inválido. Pergunte ao cliente: o paciente é pequeno (P, até 10kg), médio (M, 10-25kg), grande (G, acima de 25kg) ou extra grande (GG)?",
            }
        ...
        with get_connection() as conn:
            ...
            cur.execute(
                """
                SELECT id FROM clinica_pacientes
            ...
            if cur.fetchone():
                return {
                    "success": False,
                    "message": f"Já existe um paciente chamado {name} cadastrado.",
                }

            cur.execute(
                """
                INSERT INTO clinica_pacientes (company_id, client_id, name, species, breed, size)
                VALUES (%s, %s, %s, %s, %s, %s)
                RETURNING id
            """,
                (company_id, client_id, name, species_norm, breed_for_db, size_db),
            )
            pacienteid = cur.fetchone()["id"]

            if client_id:
                cur.execute(
                    """
                    UPDATE clients
                    SET conversation_stage = 'pacienteregistered', updated_at = NOW()
    ...
    def set_pet_size(pacientename: str, size: str) -> dict:
        ...
        if not size or not size.strip():
            return {
                "success": False,
                "message": "Porte não informado. Pergunte ao cliente: o paciente é pequeno (até 10kg), médio (10-25kg) ou grande (acima de 25kg)?",
            }
        ...
        # Tenta atualizar se o paciente já existe no banco
        updated = None
        if pacientename and pacientename.strip():
            with get_connection() as conn:
                cur = conn.cursor()
                cur.execute(
                    """
                    ...
                    RETURNING id, name
                """,
                    (size_db, company_id, client_id, pacientename),
                )
        ...
        if updated:
            return {
                "success": True,
                "pacienteupdated": True,
                "size": size_db,
                "size_label": size_label,
                "message": f"Porte de {updated['name']} confirmado como {size_label}!",
            }
        else:
            # Paciente ainda não cadastrado — retorna porte confirmado para uso posterior
            return {
                "success": True,
                "pacienteupdated": False,
                "size": size_db,
                "size_label": size_label,
                "message": f"Porte confirmado: {size_label}. Use este porte ao cadastrar o paciente e para calcular preços.",
            }

    VALID_STAGES = {"initial", "onboarding", "pacienteregistered", "booking", "completed"}
    ...
    def advance_stage(conversation_stage: str) -> dict:
        ...
        Args:
            conversation_stage: Novo estágio — valores válidos: initial, onboarding, pacienteregistered, booking, completed
        """
        ...
        with get_connection() as conn:
            ...
            cur.execute(
                ...
                SELECT
                    ...
                    p.name AS pacientename
                FROM clinica_appointments a
                JOIN clinica_services svc ON svc.id = a.service_id
                JOIN clinica_pacientes p ON p.id = a.pacienteid
                LEFT JOIN clinica_slots sl ON sl.id = a.slot_id
                LEFT JOIN clinica_schedules sch ON sch.id = a.schedule_id
