...
def build_sales_prompt(context: dict, router_ctx: dict) -> str:
    ...
    if active_paciente:
        ...
        # Nenhum paciente cadastrado — porte desconhecido
        pacientemissing_size = True
    ...
    if lodging_config.get("hotel_enabled"):
        ...
        svc_lines.append(f"  • Hotel para pacientes: {rate_str}{hours_str} — hospedagem noturna com acompanhamento")
...
O porte do paciente ainda não é conhecido. O preço dos serviços DEPENDE do porte.
...
def build_faq_prompt(context: dict, router_ctx: dict) -> str:
    ...
    pacientes = context.get("pacientes", [])
    lodging_config = context.get("lodging_config", {})
    clinica_phone = context.get("clinica_phone")
    clinica_address = context.get("clinica_address")
    client = context.get("client")
    ...
    # Serviços com preços — usa porte do paciente único se disponível
    svc_lines = []
    for s in services:
        if s.get("price_by_size"):
            sz = s["price_by_size"]
    ...
    if lodging_config.get("hotel_enabled"):
        ...
        cout = (lodging_config.get("hotel_checkout_time") or "")[:5]
        hours_str = f" (check-in {cin}, check-out {cout})" if cin and cout else ""
        svc_lines.append(f"  • Hotel para pacientes: {rate_str}{hours_str} — hospedagem noturna")
    ...
    contact_parts = []
    if clinica_phone:
        contact_parts.append(f"Telefone: {clinica_phone}")
    if clinica_address:
        contact_parts.append(f"Endereço: {clinica_address}")
    contact_text = "\n".join(contact_parts)
...
• CATÁLOGO / "O QUE VOCÊS FAZEM" / LISTA DE SERVIÇOS: ao apresentar o que o clinica oferece, inclua
...
• Conversa vaga, emoji, agradecimento, ou dúvida normal sobre clinica
