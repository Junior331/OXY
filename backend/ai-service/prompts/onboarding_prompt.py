def build_onboarding_prompt(context: dict, router_ctx: dict) -> str:
    ...
    if not pacientes:
        paciente_state = "Nenhum paciente cadastrado ainda."
    elif len(pacientes) == 1:
        p = pacientes[0]
        paciente_state = f"1 paciente cadastrado: {p['name']} ({p.get('species','?')}, {p.get('breed','?')}, porte {p.get('size','?')})."
    else:
        detail = " | ".join(
            f"{p['name']} ({p.get('species','?')}, porte {p.get('size','?')})"
            for p in pacientes
        )
        paciente_state = f"{len(pacientes)} pacientes cadastrados: {detail}."
...
• Use o nome do cliente e do paciente sempre que souber
...
• Apresente-se dizendo seu nome ({assistant_name}) e o clinica ({company_name})
• Se o cliente tem pacientes cadastrados: mencione-os pelo nome e pergunte se o atendimento é para um deles ou quer cadastrar outro
