...
export async function chatBusiness(req: Request, res: Response) {
  try {
    ...
    const [
      ...
      prisma.clinicaProfile.findUnique({
        where: { companyId },
        include: { company: true },
      }),
      prisma.clinicaAppointment.count({
      ...
      prisma.clinicaAppointment.findMany({
      ...
      prisma.clinicaPaciente.count({ where: { companyId } }),
      prisma.clinicaService.findMany({ where: { companyId, isActive: true }, select: { name: true, price: true } }),
      prisma.clinicaAppointment.findMany({
      ...
      prisma.clinicaAppointment.aggregate({
        where: { companyId, scheduledDate: { gte: monthAgo }, status: 'completed' },
        _sum: { priceCharged: true },
      }),
    ])

    const companyName = clinica?.company?.name ?? 'Clínica'
    ...
    const upcomingText = upcoming_appointments.length > 0
      ? upcoming_appointments.map((a: any) => {
          ...
          return `- ${a.client?.name ?? 'Cliente'} | Paciente: ${a.paciente?.name ?? '-'} | Serviço: ${a.service?.name ?? '-'} | ${date} ${h}`
        }).join('\n')
    ...
    const systemPrompt = `Você é um assistente de negócios especializado em clínicas veterinárias.
