...
async function buildAlerts(companyId: number): Promise<BrainAlert[]> {
  ...
  if (pacienteBirthdays.status === 'fulfilled' && pacienteBirthdays.value.length > 0) {
    ...
    alerts.push({
      ...
      action: 'Quais pacientes fazem aniversário essa semana?',
    })
  }
...
export async function buildContext(companyId: number): Promise<BrainContext> {
  ...
  const [company, profile, revenueRaw, appointmentsRaw, clientsRaw, conversionRaw, alertsResult] = await Promise.all([
    ...
    prisma.$queryRaw<Array<{ assistant_name: string | null }>>`
      SELECT assistant_name FROM clinica_profile WHERE company_id = ${companyId} LIMIT 1
    `,
  ...
  return {
    clinica_name:                   company[0]?.name ?? 'Clínica',
...
export function contextToSystemPrompt(ctx: BrainContext): string {
  ...
  return `Você é ${ctx.assistant_name}, o assistente inteligente do ${ctx.clinica_name}.
Você ajuda o dono do clinica a entender seu negócio respondendo perguntas em linguagem natural.
...
export const BRAIN_SUGGESTION_FALLBACKS = [
  ...
  'Quais pacientes fazem aniversário nos próximos 7 dias?',
