...
export const TOOLS = [
  {
    ...
    function: {
      ...
      description: 'Retorna a lista completa de agendamentos de hoje com nome do cliente, paciente, serviço e status de confirmação.',
  ...
  },
  {
    ...
    function: {
      ...
      description: 'Retorna os próximos agendamentos futuros do clinica.',
      parameters: {
        type: 'object',
        properties: {
  ...
  },
  {
    ...
    function: {
      ...
      description: 'Busca informações de um cliente pelo nome: pacientes, histórico de visitas, último agendamento.',
      parameters: {
        type: 'object',
        properties: {
...
export async function executeTool(name: string, args: any, companyId: number): Promise<string> {
  switch (name) {
    ...
    case 'get_lost_clients': {
      ...
      const data = await prisma.$queryRaw<Array<{ client_name: string; pacientename: string; last_visit: string | null; days_absent: number }>>`
        SELECT client_name, pacientename, last_visit, days_absent
      ...
      return `${data.length} cliente(s) sumidos há mais de ${minDays} dias:\n` +
        data.map(c =>
          `• ${c.client_name} (${c.pacientename}) — última visita: ${c.last_visit ? new Date(String(c.last_visit) + 'T12:00:00').toLocaleDateString('pt-BR') : 'nunca'} (${c.days_absent} dias atrás)`
        ).join('\n')
    }
    ...
    case 'get_client_info': {
      ...
      const pacientes = await prisma.$queryRaw<Array<{ name: string; species: string; breed: string | null; size: string | null }>>`
      ...
      try {
        ...
        return `${data.length} paciente(s) fazem aniversário nos próximos ${days} dias:\n` +
          data.map(p => {
            ...
            return `• ${p.pacientename} (tutor: ${p.client_name}) — aniversário: ${bday} — em ${p.days_until} dia(s)`
          }).join('\n')
      } catch {
        return `Nenhum paciente faz aniversário nos próximos ${days} dias.`
      }
    }
