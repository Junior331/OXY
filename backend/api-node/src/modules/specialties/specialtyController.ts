...
/**
 * Builds the Prisma.Sql row fragments for a bulk INSERT into clinica_slots.
...
export async function listSpecialties(req: Request, res: Response) {
  try {
    ...
    const specialties = await prisma.clinicaSpecialty.findMany({
      where,
      orderBy: { name: 'asc' },
    })
...
export async function getSpecialty(req: Request, res: Response) {
  try {
    ...
    const specialty = await prisma.clinicaSpecialty.findUnique({
      where: { id },
      include: {
...
export async function createSpecialty(req: Request, res: Response) {
  try {
    ...
    const specialty = await prisma.clinicaSpecialty.create({
...
export async function updateSpecialty(req: Request, res: Response) {
  try {
    ...
    const existing = await prisma.clinicaSpecialty.findUnique({ where: { id } })
    ...
    const specialty = await prisma.clinicaSpecialty.update({ where: { id }, data })

    if (is_active === false) {
      await prisma.clinicaService.updateMany({
...
export async function deleteSpecialty(req: Request, res: Response) {
  try {
    ...
    await prisma.clinicaSpecialty.update({ where: { id }, data: { isActive: false } })
    await prisma.clinicaService.updateMany({
      where: { specialtyId: id, companyId },
      data: { isActive: false },
    })
...
export async function listCapacityRules(req: Request, res: Response) {
  try {
    ...
    const [specialty, rules] = await Promise.all([
      prisma.clinicaSpecialty.findUnique({ where: { id: specialtyId }, select: { companyId: true } }),
...
export async function upsertCapacityRule(req: Request, res: Response) {
  try {
    ...
    const specialty = await prisma.clinicaSpecialty.findUnique({ where: { id: specialtyId } })
    if (!specialty || specialty.companyId !== companyId) {
      return res.status(404).json({ error: 'Specialty not found' })
    }
...
// clinica_business_hours opening time for that weekday (or "08:00" default).
...
export async function bulkUpsertCapacityRules(req: Request, res: Response) {
  try {
    ...
    if (slotRows.length > 0) {
      await prisma.$executeRaw`
        INSERT INTO clinica_slots (id, company_id, specialty_id, slot_date, slot_time, max_capacity, used_capacity)
