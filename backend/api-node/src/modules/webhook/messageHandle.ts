...
// ─────────────────────────────────────────
// Notifica owner do clinica quando cliente é escalado
// ─────────────────────────────────────────
async function _notifyEscalationToOwner(
  ...
  const ownerJid = `${clinica.ownerPhone.replace(/\D/g, '')}@s.whatsapp.net`
  await sendTextMessage(String(companyId), ownerJid, message)
  console.log(`[Escalation][company:${companyId}] Notificação enviada para ${clinica.ownerPhone}`)
}
