...
function shapePet(paciente: any) {
  const birthDate = paciente.birthDate
    ...
    : null
  const weightKg = paciente.weightKg != null ? Number(paciente.weightKg) : null

  return {
    ...paciente,
    birthDate,
    birth_date: birthDate,
    age: calculateAge(paciente.birthDate),
    weightKg,
    weight_kg: weightKg,
    weight: weightKg,
    size: paciente.size ?? null,
    medical_info: paciente.notes ? { conditions: [paciente.notes] } : null,
...
export async function listPets(req: Request, res: Response) {
  try {
    ...
    const pacientes = await prisma.clinicaPaciente.findMany({
      where,
...
export async function getPet(req: Request, res: Response) {
  try {
    ...
    if (!paciente || paciente.companyId !== companyId) {
    ...
    res.status(500).json({ error: 'Failed to get paciente' })
  }
}
...
export async function createPet(req: Request, res: Response) {
  try {
    ...
    const paciente = await prisma.clinicaPaciente.create({
      data: {
        companyId,
