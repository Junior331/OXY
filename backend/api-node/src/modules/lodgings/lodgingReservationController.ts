...
function shapeLodgingReservation(r: any) {
  return {
    ...
    pacienteid: r.paciented,
    pacientename: r.paciente?.name ?? null,
    pacientebreed: r.paciente?.breed ?? null,
    pacientesize: r.paciente?.size ?? null,
    type: r.type,
...
const reservationInclude = {
  ...
  paciente: { select: { name: true, breed: true, size: true } },
}
...
/**
 * Paciente occupies calendar day D iff checkin_date <= D AND checkout_date > D (checkout exclusive).
...
export async function listLodgingReservations(req: Request, res: Response) {
  try {
    ...
    const { status, type, client_id, pacienteid, checkin_from, checkin_to } = req.query
    ...
    const reservations = await prisma.clinicaLodgingReservation.findMany({
      where,
...
export async function getLodgingReservation(req: Request, res: Response) {
  try {
    ...
    const reservation = await prisma.clinicaLodgingReservation.findUnique({
      where: { id: id! },
      include: reservationInclude,
    })
...
export async function createLodgingReservation(req: Request, res: Response) {
  try {
    ...
    if (!client_id || !pacienteid || !type || !checkin_date || !checkout_date) {
      return res.status(400).json({
        error: 'client_id, pacienteid, type, checkin_date e checkout_date são obrigatórios',
      })
    }
    ...
    const reservation = await prisma.$transaction(async (tx) => {
      ...
      const caps = await tx.clinicaLodgingCapacity.findMany({
      ...
      const overlapping = await tx.clinicaLodgingReservation.findMany({
        where: {
          companyId,
          type,
      ...
      return tx.clinicaLodgingReservation.create({
        data: {
          companyId,
          clientId: client_id,
...
export async function updateLodgingReservation(req: Request, res: Response) {
  try {
    ...
    const existing = await prisma.clinicaLodgingReservation.findUnique({ where: { id: id! } })
    ...
    const reservation = await prisma.clinicaLodgingReservation.update({
      where: { id: id! },
      data,
      include: reservationInclude,
    })
...
export async function cancelLodgingReservation(req: Request, res: Response) {
  try {
    ...
    await prisma.clinicaLodgingReservation.update({
      where: { id: id! },
