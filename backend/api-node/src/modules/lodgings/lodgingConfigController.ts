...
export async function getLodgingConfig(req: Request, res: Response) {
  try {
    ...
    const [config, capacities] = await Promise.all([
      prisma.clinicaLodgingConfig.findUnique({ where: { companyId } }),
      prisma.clinicaLodgingCapacity.findMany({
        where: { companyId },
...
export async function upsertLodgingConfig(req: Request, res: Response) {
  try {
    ...
    const config = await prisma.clinicaLodgingConfig.upsert({
      where: { companyId },
      create: { companyId, ...data },
      update: { ...data, updatedAt: new Date() },
    })

    const capacities = await prisma.clinicaLodgingCapacity.findMany({
      where: { companyId },
      orderBy: [{ type: 'asc' }, { dayOfWeek: 'asc' }],
    })
...
export async function upsertLodgingCapacity(req: Request, res: Response) {
  try {
    ...
    const upsertPromises = capacities.map((entry: any) => {
      ...
      return prisma.clinicaLodgingCapacity.upsert({
        where: {
    ...
    const updatedCapacities = await prisma.clinicaLodgingCapacity.findMany({
      where: { companyId },
      orderBy: [{ type: 'asc' }, { dayOfWeek: 'asc' }],
    })
