...
function shapeLodging(l: any) {
  return {
    ...
    pacienteid: l.paciented,
    pacientename: l.paciente?.name ?? null,
...
export async function listLodgings(req: Request, res: Response) {
  try {
    ...
    const { status, client_id, pacienteid, checkin_from, checkin_to } = req.query
    ...
    const lodgings = await prisma.clinicaLodging.findMany({
      where,
...
export async function getKennelAvailability(req: Request, res: Response) {
  try {
    ...
    const occupied = await prisma.clinicaLodging.findMany({
      where: {
        companyId,
      ...
      select: { kennelId: true, paciente: { select: { name: true } } },
    })

    const occupiedKennels = occupied.map(l => ({
      kennel_id: l.kennelId,
      pacientename: l.paciente?.name,
    }))
...
export async function getLodging(req: Request, res: Response) {
  try {
    ...
    const lodging = await prisma.clinicaLodging.findUnique({
      where: { id: id! },
      include: lodgingInclude,
    })
...
export async function createLodging(req: Request, res: Response) {
  try {
    const companyId = req.user!.companyId
    const { client_id, pacienteid, specialty_id, kennel_id, checkin_date, checkout_date, checkin_time, checkout_time, daily_rate, care_notes, emergency_vet, notes } = req.body

    if (!client_id || !pacienteid || !checkin_date || !checkout_date) {
      return res.status(400).json({ error: 'client_id, pacienteid, checkin_date e checkout_date são obrigatórios' })
    }
    ...
    const lodging = await prisma.clinicaLodging.create({
      data: {
        companyId,
        clientId: client_id,
...
export async function updateLodging(req: Request, res: Response) {
  try {
    ...
    const existing = await prisma.clinicaLodging.findUnique({ where: { id: id! } })
    ...
    const lodging = await prisma.clinicaLodging.update({
      where: { id: id! },
      data,
      include: lodgingInclude,
    })
...
export async function cancelLodging(req: Request, res: Response) {
  try {
    ...
    await prisma.clinicaLodging.update({
      where: { id: id! },
