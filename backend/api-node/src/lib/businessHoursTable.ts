...
/** Linha de `clinica_business_hours` (nomes alinhados ao SQL raw). */
...
/**
 * JSON no formato que o frontend já esperava (`businessHours` no clinica).
...
export async function attachBusinessHoursToClinicaJson<T extends { companyId: number }>(
  clinica: T,
): Promise<T & { businessHours: BusinessHoursClientJson }> {
  const rows = await loadBusinessHourRows(clinica.companyId)
  return { ...clinica, businessHours: businessHoursRowsToClientJson(rows) }
}
