  ...
  return (
    <Tooltip.Provider delayDuration={200}>
      <div className="flex w-full items-center justify-between gap-2 px-1">
        {/* Mobile menu button (only shows when onMenuClick provided and on mobile) */}
        {onMenuClick && (
          <button
            type="button"
            onClick={onMenuClick}
            className={cn(
              "flex h-[38px] w-[38px] shrink-0 items-center justify-center rounded-full transition-colors lg:hidden",
              "border border-[#727B8E]/10 bg-white backdrop-blur-[6px] dark:border-[#40485A] dark:bg-[#1A1B1D]/90",
            )}
            aria-label="Abrir menu"
          >
            <Menu
              className="h-5 w-5 stroke-[#8A96A8] dark:stroke-[#8a94a6]"
              strokeWidth={1.67}
            />
          </button>
        )}
