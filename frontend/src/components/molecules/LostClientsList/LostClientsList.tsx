...
function openWhatsApp(phone: string, clientName: string, pacienteName: string) {
  const cleaned = phone.replace(/\D/g, '')
  const number = cleaned.startsWith('55') ? cleaned : `55${cleaned}`
  const msg = encodeURIComponent(
    `Olá ${clientName}! Sentimos falta do(a) ${pacienteName} por aqui 🐾 Quando conseguimos agendar o próximo banho?`,
  )
            ...
            >
              <div className="min-w-0 flex-1">
                <p className="truncate text-sm font-medium text-[#111827] dark:text-[#f5f9fc]">
                  {client.client_name} · {client.paciente_name}
                </p>
              ...
              <div className="ml-3 flex items-center gap-2 shrink-0">
                ...
                <button
                  onClick={() => openWhatsApp(client.phone, client.client_name, client.paciente_name)}
