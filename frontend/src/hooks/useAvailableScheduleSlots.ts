 ...
 * Sem cache em memória: cada execução do efeito chama a API de novo (data/serviço/paciente/modal),
 ...
 * @param pacienteId - Optional paciente UUID; com multiplier e porte G/GG, filtra pares de slots
 */
export function useAvailableScheduleSlots(
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    const cacheKey = `${dateIso}|${serviceId ?? ""}|${pacienteId ?? ""}`;
  ...
  }, [dateIso, serviceId, pacienteId, enabled]);
