...
function appointmentToCalendarEvent(a: Appointment): CalendarEvent {
  ...
  const pacienteName = a.paciente_name || a.client_name || "Agendamento";
  ...
  return {
    id: a.id,
    pacienteName,
    pacienteInitials: getInitials(pacienteName),
...
export default function CalendarioPage() {
  ...
  /** Com modal aberto + serviço/paciente, o mês reflete par de slots (G/GG + multiplier). */
  const monthAvailabilityContext = useMemo(() => {
    if (!isModalOpen) return "";
    return `${formData.serviceId ?? ""}|${formData.pacienteId ?? ""}`;
  }, [isModalOpen, formData.serviceId, formData.pacienteId]);
            ...
            toFetch.map(async ({ y, m, key }) => {
              const result = await appointmentService.getAvailableDates({
                ...
                ...(isModalOpen && formData.serviceId
                  ...
                  : {}),
                ...(isModalOpen && formData.pacienteId
  ...
  const handleSubmit = async () => {
    ...
    if (!formData.pacienteId) errs.pacienteId = "Selecione um paciente";
              ...
              {showNewPacienteForm ? (
                ...
                <div className="flex items-center gap-2 rounded-lg border border-[#727B8E]/20 bg-[#F4F6F9] p-3 dark:border-[#40485A] dark:bg-[#212225]">
                  ...
                  <span className="text-sm text-[#727B8E] dark:text-[#8a94a6]">
                    Carregando pacientes do cliente...
                  </span>
                </div>
              ) : (
                <div>
                  ...
                  {formErrors.pacienteId && (
                    <p className="mt-1 text-xs text-red-500">{formErrors.pacienteId}</p>
                  )}
                </div>
              )}
            </div>
