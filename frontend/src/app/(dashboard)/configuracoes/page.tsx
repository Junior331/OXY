...
import type { Specialty } from "@/types/clinica";
import type { Service } from "@/types";
import type { CapacityRule } from "@/types/clinica";
...
function getDayBHInfo(
  bh: Clinica["businessHours"] | undefined,
  dayOfWeek: number,
...
function HourlyCapacityEditor({
  ...
  businessHours: Clinica["businessHours"] | undefined;
  config: Record<number, DaySlotConfig>;
...
function SettingsProfileSidebar({
  ...
  const establishmentName =
    clinica?.company?.name ?? clinica?.assistantName ?? "Estabelecimento";
  const displayPhone = clinica?.phone || clinica?.ownerPhone;
...
function EmpresaContent({
  ...
  useEffect(() => {
    if (!clinica) return;
    setNome(clinica.company?.name || "");
    setTelefone(clinica.phone || "");
    ...
    setOwnerPhone(normalizePhone(clinica.ownerPhone || ""));
    setEmergencyContact(normalizePhone(clinica.emergencyContact || ""));
    setAddress({
      cep: clinica.cep || "",
      rua: clinica.address || "",
      numero: "",
...
function IAPlaygroundContent() {
  const [messages, setMessages] = useState<ChatMessage[]>([
    {
      ...
      role: "assistant",
      content:
        "Olá! Sou a IA do seu clinica. Como posso ajudar você hoje? Você pode me perguntar sobre agendamentos, serviços, preços ou qualquer dúvida que seus clientes possam ter.",
      timestamp: new Date(),
    },
  ]);
  ...
  const handleSendMessage = async () => {
    ...
    setTimeout(
      () => {
        const responses = [
          "Entendi! Posso ajudar você com isso. Para agendar um serviço, basta informar o nome do paciente, o tipo de serviço desejado e a data/horário de preferência.",
          "Nosso clinica oferece diversos serviços como banho, tosa, consultas veterinárias e hospedagem. Qual serviço você gostaria de conhecer melhor?",
          ...
          "Para remarcar um agendamento, preciso do nome do paciente e da data atual do agendamento. Com essas informações, consigo verificar a disponibilidade de novos horários.",
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-[#727B8E] dark:text-[#8a94a6] mt-0.5">
              Hospedagem noturna para pacientes.
            </p>
          </div>
      ...
      <div className="rounded-xl border border-[#727B8E]/10 bg-white dark:border-[#40485A] dark:bg-[#1A1B1D] p-5 flex flex-col gap-4">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-[#727B8E] dark:text-[#8a94a6] mt-0.5">
              Cuidados diurnos e atividades para pacientes.
            </p>
          </div>
          <label className="relative inline-flex items-center cursor-pointer">
            <input
          ...
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <p className="text-xs text-[#727B8E] dark:text-[#8a94a6] mt-0.5">
                ...
                carregar, usa-se o horário salvo no cadastro do clinica.
              </p>
            </div>
          </div>
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="flex items-center justify-between rounded-lg border border-[#727B8E]/10 bg-[#F4F6F9] dark:border-[#40485A] dark:bg-[#212225] px-4 py-3">
            <div className="flex-1">
              ...
              >
                Pacientes de porte grande (G e GG) ocupam o dobro do tempo
              </label>
          ...
          <div className="flex items-center justify-between rounded-lg border border-[#727B8E]/10 bg-[#F4F6F9] dark:border-[#40485A] dark:bg-[#212225] px-4 py-3">
            <div className="flex-1">
              <label
                ...
                className="cursor-pointer text-sm font-medium text-[#434A57] dark:text-[#f5f9fc]"
              >
                Pacientes G/GG ocupam o dobro do tempo
              </label>
              <p className="text-xs font-normal text-[#727B8E]">
                Reserva dois slots consecutivos para porte G/GG
              </p>
